...
| R-12 | 名次奖励经济（替代 R-09）：取消开局押注。每位玩家起始 100 货币 + 2 生命。每局结算按名次由系统发放金钱：第一 +40、中间 +10、垫底 0；并列组按区间奖励平分（垫底组整体 0）。垫底（含并列）扣 1 生命，生命为 0 出局观战；手牌超限失败者与首回合 Kongbaya 失败者按垫底处理。货币不扣、下限 0，出局者保留货币参与把末排名；把末与商店沿用 R-08/R-10/R-11。 | 取消押注博弈，改为直观的名次发钱 + 生命淘汰；商店仍是唯一货币回收点。 |
