...
> **本文档职责**：游戏**规则、产品、架构的权威主规格**（含冻结规则 R-01~R-11）。开发/实现细节见 `功能实现文档.md` 与 `AI_AGENT_交接说明.md`；协议契约见 `网络协议_V1.md`。改动规则前必须先更新本文档。
...
- 最低者获胜；最高者（垫底）失去押注（货币），若该玩家为 all-in 则灵魂币 `health→0` 出局；其他非最后玩家按 1.5× 返还、第一名(并列平分) 2× 返还并拿走垫底者押注，差额由系统补足（见 §3.7 R-09）。
- 若喊话者在其首次行动前就喊 Kongbaya，且最终并非最低者，则按"最后一名"处理（失去押注，all-in 则出局）。
- 手牌超限（R-07）：任一玩家手牌总数超过 6 张时对局立即结束，该玩家判定失败并扣 1 点生命，其余玩家按各自手牌点数结算排名。
...
| R-07 | 任一玩家手牌总数超过 6 张时，对局立即结束，该玩家判定失败并按"最后一名"处理（失去押注；若 all-in 则灵魂币 `health→0` 出局）；其余玩家按各自手牌点数结算排名（最低分者胜），失败玩家不参与排名。 | 手牌超限由罚抽（贴错）累积产生，为对局提供确定性终止与失败判定。 |
...
**规则变更流程：**任何影响 R-01 至 R-11 的修改，必须先新增决策 ID、说明旧/新裁定、补测试项，再调整实现；不能以 UI 方便或局部 bug 为理由静默修改规则。
...
| 当前阶段、人数、牌数 | 完整 | 公开 | 公开 |
| 结算牌面 | 完整 | 公开 | 公开 |
| 货币（currency）、灵魂币（health） | 完整 | 公开（各自当前值） | 公开（各自当前值） |
