...
> **本文档职责**：游戏**规则、产品、架构的权威主规格**（含冻结规则 R-01~R-12）。开发/实现细节见 `功能实现文档.md` 与 `AI_AGENT_交接说明.md`；协议契约见 `网络协议_V1.md`。改动规则前必须先更新本文档。
...
4. 每当一张牌进入弃牌堆，进入贴牌窗口（窗口延续到下一玩家抽牌为止）；
5. 下一玩家抽牌即关闭贴牌窗口；贴中者按 3.5 立即解决。

### 3.4 技能裁定

- 7/8：操作者指定自己的一个位置，定向看到对应牌面一次。
- 9/10：操作者指定其他玩家的一个位置，定向看到对应牌面一次。
- J：操作者选择一名其他玩家及其一个位置、自己的一个位置；服务器直接交换两张牌，过程中不显示牌面。
...
当新牌进入弃牌堆，服务器开启贴牌窗口并**立即推进到下一位玩家的抽牌阶段**；窗口一直保持到**下一位玩家抽牌**为止（无固定时长）。窗口期间所有玩家可点击任意一张暗牌进行尝试；下一玩家抽牌即关闭窗口。

| 情况 | 服务器行为 |
| --- | --- |
| 点数不匹配 | 该玩家罚抽 1 张；窗口继续，可再次尝试；若罚抽后该玩家手牌总数超过 6 张，对局立即结算，该玩家判定失败 |
| 正确（首个进入 400ms 收集窗） | 收集窗内其他正确贴牌一并记录；结束后仅 1 人则按下列两条解决 |
| 收集后唯一正确，且目标是自己 | 目标牌进入弃牌堆；该玩家手牌数 -1；窗口结束 |
| 收集后唯一正确，且目标是他人 | 窗口结束；贴牌者选自己一张牌交给目标玩家；被贴中的目标牌进入弃牌堆 |
| 收集后 ≥2 人正确 | 进入**比拼**：随机加粗区中心红心为目标，各候选人按 STOP，最接近红心者按上述"正确"规则解决；败者无惩罚 |
| 正确但窗口已因抽牌/他人贴中关闭 | 不处罚；该请求被忽略 |
| 无人正确 | 下一玩家抽牌时窗口自然关闭 |

贴牌解决后，**恢复当前玩家的抽牌阶段**（他尚未抽牌，贴牌不打断其回合）。

> **时序细节（贴牌动画）**：贴牌尝试时服务器把被贴的牌公开翻给所有玩家，并附 `correct` 对错标记（客户端据此打绿/红炫光）。正确贴牌在结算时（`slap_resolved`）把被贴的牌**立即**移入弃牌堆——即使是"目标为他人"的情况，也在进入交换阶段之际就清出该槽位并弃牌，随后交换阶段只把贴牌者的牌补进已清空的槽位（`slap_gift`）。最终状态与旧逻辑一致（对方槽位=贴牌者的牌、弃牌堆含被贴牌），只是弃牌时机提前到结算，以匹配动画与状态。贴错时服务器广播 `slap_penalty`（含落位槽号），客户端把罚牌从抽牌堆飞到该槽位；罚牌优先填入空槽，手牌满时作为第 5+ 张渲染在**手牌上方独立行**（前 4 张槽位永不位移）。

“第一个/最接近”以权威服务器接收与验证的先后、以及服务器记录的 STOP 到达时间判定，不使用客户端本地时间。这样 LAN 和未来 VPS 都有一致结果。

### 3.6 Kongbaya 与结算

- 仅当前玩家在自己行动开始（`TurnDraw`）可喊 Kongbaya。
- **一场对局只允许喊一次**：已有人喊过后（`kong_caller != -1`）任何玩家再喊均被拒绝；最终轮玩家也不可再喊。
- 喊出者不再进行普通行动；按顺时针顺序，其他每位玩家各执行一轮完整最终行动。
- 最终行动仍可触发特殊牌和贴牌。
- 随后翻开所有牌，以以下顺序从低到高排列：总分 → 牌数量 → 从最大单牌开始逐一比较。数值更小者靠前。
- 最低者获胜；按最终名次由系统发放金钱：第一 **+40**、中间名次 **+10**、垫底 **0**；并列组按区间奖励平分（垫底组整体 0），全员同分则各 +10（见 §3.7 R-12）。
- **垫底（含并列垫底）扣 1 生命**，生命为 0 出局观战；若喊话者在其首次行动前就喊 Kongbaya，且最终并非最低者，则按"垫底"处理（本局 0 金钱、−1 生命）。
- 手牌超限（R-07）：任一玩家手牌总数超过 6 张时对局立即结束，该玩家判定失败并按"垫底"处理（本局 0 金钱、−1 生命），其余玩家按各自手牌点数结算排名。
...
| 决策 ID | 最终裁定 | 原因 |
| --- | --- | --- |
| R-01 | 同一窗口内玩家可**多次**尝试贴牌；贴错每次罚抽一张且可继续尝试；贴对进入判定（多人同时则比拼，见 3.5）；判定翻牌期间不能点击新牌。 | 先到正确者胜 / 同时比拼，贴牌仍保留风险。 |
...
| R-06 | 普通行动和 Q 决策在 LAN MVP 没有倒计时；贴牌窗口也没有固定时长，由“下一玩家抽牌”关闭。 | 优先验证规则和交互，不因网络/思考时间误判。 |
| R-07 | 任一玩家手牌总数超过 6 张时，对局立即结束，该玩家判定失败并按"垫底"处理（本局 0 金钱、−1 生命，见 R-12）；其余玩家按各自手牌点数结算排名（最低分者胜），失败玩家不参与排名。 | 手牌超限由罚抽（贴错）累积产生，为对局提供确定性终止与失败判定。 |
| R-08 | 系列赛：房主开局设定局数 X（2-10，默认 5）。每局结算后（GAME_OVER）非把末时由服务器自动开下一局；赛满 X 局或存活数≤1 时结束系列赛。`health==0` 者出局观战（保留座位、仅公开信息、不可操作）。把末总排名：存活者按胜场数降序→同胜场按货币降序，淘汰者不入榜。 | 把单局扩展为多局系列赛，死亡玩家不踢出房间。 |
| R-09 | **已作废（见 R-12）。** 押注经济：起始 100 货币 + 1 灵魂币（health，默认 1）。每局开局确认后进入押注阶段：存活玩家押注（入场 20，+5 递增，上限 100 且 ≤ 当前货币）。结算：第一名(并列平分) 2× 返还 + 拿走垫底者全部押注；其他非最后玩家 1.5× 返还；最后一名押注全失；差额由系统补足。货币 < 入场时自动 all-in（押全部货币 + 灵魂币），all-in 且垫底则 health→0 出局。并列垫底同失。 | 货币驱动多局系列赛：低分获胜 + 押注风险，灵魂币只在 all-in 失败时扣。 |
| R-10 | 商店固定价：系列赛非把末结算后进入商店，展示至多 3 件随机遗物（v1 池 2 件，各固定价 50）。每位存活玩家**限购 1 件**：点击遗物且货币 ≥ 售价即扣款并立即获得（同种遗物覆写不累积），同件**先到先得**（售出后他人不可再买）；也可「跳过购买」。全员完成（购买或跳过）→ 开下一局；货币不足只能跳过。把末不进商店。 | 局间货币消费点：遗物强化能力，固定价降低上手成本。 |
| R-11 | 遗物效果 v1：持有 **Joker 变换** 遗物者，抽牌阶段（抽牌堆或弃牌堆顶）抽到 Joker 时可选变换为任意牌（点数+花色，随后按正常抽牌流程处理）；**未变换**时其手中 JOKER 分值按 +2 计（结算与揭示一致）。持有 **防守护盾** 遗物者，购买后在**下一局开局**随机保护一格（一局后失效），该格**免疫其他玩家的贴牌与 J/Q 交换**（看牌与自己操作不受影响）。 | 遗物增强记忆/换牌/贴牌路线：不暴露隐藏信息、不改变贴牌服务器先后顺序；基础模式（未购遗物）零影响。 |
| R-12 | 名次奖励经济（替代 R-09）：取消开局押注。每位玩家起始 100 货币 + 2 生命。每局结算按名次由系统发放金钱：第一 +40、中间 +10、垫底 0；并列组按区间奖励平分（垫底组整体 0）。垫底（含并列）扣 1 生命，生命为 0 出局观战；手牌超限失败者与首回合 Kongbaya 失败者按垫底处理。货币不扣、下限 0，出局者保留货币参与把末排名；把末与商店沿用 R-08/R-10/R-11。**淘汰延后一局生效**：某玩家在结算中生命归零时，本局仍按正常参与者结算与展示，从**下一局**起才进入观战。**仅一名玩家参与排名时不算垫底**，不扣生命、不惩罚（如手牌超限后仅剩一人）。 | 取消押注博弈，改为直观的名次发钱 + 生命淘汰；淘汰延后避免"本局还没结算就被判观战"；商店仍是唯一货币回收点。 |

**规则变更流程：**任何影响 R-01 至 R-12 的修改，必须先新增决策 ID、说明旧/新裁定、补测试项，再调整实现；不能以 UI 方便或局部 bug 为理由静默修改规则。
...
对局结束（`GAME_OVER`）后客户端弹出**居中浮动排名窗口**（不遮挡四块玩家手牌区，棋盘卡牌保持可见），按**同步轮记分**展示结算：

- 结算时棋盘所有手牌**初始为背面**（不再一次性全亮），由算分动画**逐张联动翻面**。
- 每轮所有玩家**同时**翻开各自第 N 张牌：**棋盘上对应卡牌纵轴翻到正面**，同时结算页行内累计分数字滚动累加（低分=冠军置顶）。棋盘翻牌与行内揭示**同一时刻触发**、逐张推进（每张间隔 `FLIP_STEP`）。
- 每轮翻完，排名表整行做**纵向上升/下降位移动画**实时重排（复用 `ScoreSystem` 排序：总分→牌数→从小到大逐张比→id 兜底）。
- 全部翻完后，冠军行名字播放**金色脉冲光环 + ★ 徽章**，并播放胜利音效（Winner01-04 随机）。
- 出口：房主可「再来一局」（见协议 `next_match`，`GAME_OVER` 后直接开新局，`match_number` 递增）或「返回大厅」；客户端显示「等待房主开始下一局」。
- 仅以快照 `result.ranking` 为空时（reason-only 结算，如全员离开）不弹结算页，沿用右下角摘要。
...
| 7/8 | 点击自己的目标牌 |
| 9/10/Q | 点击其他玩家的目标牌 |
| J | 依次点击对方的牌（选定目标+牌位）、自己要换的牌，两次点击即完成交换 |
...
| 当前阶段、人数、牌数 | 完整 | 公开 | 公开 |
| 结算牌面 | 完整 | 公开 | 公开 |
| 货币（currency）、生命（health） | 完整 | 公开（各自当前值） | 公开（各自当前值） |
...
| 贴牌 | 目标玩家、槽位 | 贴牌窗口（`slap_open`）、槽位合法 |
| 贴牌比拼 STOP | 无 | 仅比拼候选人、未按过 STOP |
...
6. 系列赛中出局玩家（`health==0`）断线重连后仍为观战者：保留座位、仅公开信息、不可操作。
...
| T13 | 贴错 | 罚该玩家一张，窗口继续，可再次尝试 |
| T14 | 两人同时正确贴 | 以服务器最先接收的正确请求为成功者，后到者不受罚 |
