...
> **本文档职责**：游戏**规则、产品、架构的权威主规格**（含冻结规则 R-01~R-06）。开发/实现细节见 `功能实现文档.md` 与 `AI_AGENT_交接说明.md`；协议契约见 `网络协议_V1.md`。改动规则前必须先更新本文档。
...
| R-07 | 任一玩家手牌总数超过 6 张时，对局立即结束，该玩家判定失败并扣 1 点生命；其余玩家按各自手牌点数结算排名（最低分者胜），失败玩家不参与排名。 | 手牌超限由罚抽（贴错）累积产生，为对局提供确定性终止与失败判定。 |
...
**规则变更流程：**任何影响 R-01 至 R-08 的修改，必须先新增决策 ID、说明旧/新裁定、补测试项，再调整实现；不能以 UI 方便或局部 bug 为理由静默修改规则。
