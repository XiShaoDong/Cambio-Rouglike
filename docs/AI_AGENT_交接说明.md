...
9. **断线 = 标记离线 + 条件暂停，不中止对局**：非当前行动者掉线游戏继续；当前行动者（或开局记忆阶段任一玩家）离线时 `suspended=true` 暂停并拒绝所有操作（`MATCH_SUSPENDED`）。房主可 `request_kick_offline(seat)` 踢出继续 / `request_abort_match` 中止回大厅 / `request_close_room` 解散房间（关服务器回初始界面）。**踢出后剩余人数 < MIN_PLAYERS 自动中止回大厅**。玩家 token 重连（`request_reconnect`）在阶段二实现。
